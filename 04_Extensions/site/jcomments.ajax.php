...
class JCommentsAJAX
{
	...
	public static function showForm( $object_id, $object_group, $target )
	{
		...
		$oldScript = isset($data['script']['text/javascript']) ? $data['script']['text/javascript'] : '';

		$form = JComments::getCommentsForm($object_id, $object_group);
		$response->addAssign($target, 'innerHTML', $form);

		$data = JFactory::getDocument()->getHeadData();
		$script = isset($data['script']['text/javascript']) ? $data['script']['text/javascript'] : '';

		$addedScript = trim(str_replace($oldScript, '', $script));
		if(!empty($addedScript)) {
			$addedScript = str_replace("\n", '', $addedScript);
			$addedScript = str_replace("\r", '', $addedScript);
			$response->addScript($addedScript);
		}
				...
				if ($acl->check('enable_captcha') == 1) {
					...
					if ($captchaEngine == 'kcaptcha') {
						...
						if (($captcha = JCaptcha::getInstance($plugin, array('namespace' => 'jcomments'))) !== null) {
							if ($plugin == 'recaptcha') {
								$response->addScript("try{Recaptcha.reload();}catch(ex){}");
							}

							$value = isset($values['captcha']) ? $values['captcha'] : null;
							if (!$captcha->checkAnswer($value)) {
								self::showErrorMessage(JText::_('ERROR_CAPTCHA'));
								return $response;
							}
						} else {
							self::showErrorMessage(JText::_('JLIB_CAPTCHA_ERROR_PLUGIN_NOT_FOUND'));
							return $response;
						}
					}
				}
