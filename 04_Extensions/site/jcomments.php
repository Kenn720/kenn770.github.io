	...
	public static function getCommentsForm($object_id, $object_group, $showForm = true)
	{
		...
		if ($acl->canComment()) {
			...
			if (!$showForm) {
				...
				// we need include scripts which are used for captcha
				if ($acl->check('enable_captcha') == 1) {
					$captchaEngine = $config->get('captcha_engine', 'kcaptcha');
					if ($captchaEngine != 'kcaptcha') {
						$plugin = $captchaEngine == 'joomladefault' ? JFactory::getConfig()->get('captcha') : $captchaEngine;
						if (($captcha = JCaptcha::getInstance($plugin, array('namespace' => 'jcomments'))) !== null) {
							$oldData = JFactory::getDocument()->getHeadData();
							$oldScripts = array_keys($oldData['scripts']);
       					
       							$captcha->display('captcha', 'comments-form-captcha', '');

							$newData = JFactory::getDocument()->getHeadData();
							$newScripts = array_keys($newData['scripts']);

							JFactory::getDocument()->setHeadData($oldData);

							$addedScripts = array_diff_assoc($newScripts, $oldScripts);
			...
			if ($acl->check('enable_captcha') == 1) {
				...
				if ($captchaEngine == 'kcaptcha') {
					...
					$plugin = $captchaEngine == 'joomladefault' ? JFactory::getConfig()->get('captcha') : $captchaEngine;
					if (($captcha = JCaptcha::getInstance($plugin, array('namespace' => 'jcomments'))) !== null) {
						$captchaHTML = $captcha->display('captcha', 'comments-form-captcha', '');
						$captchaHTML = sprintf('<div id="comments-form-captcha-holder">%s</div>', $captchaHTML);

						$tmpl->addVar('tpl_form', 'comments-form-captcha', 1);
						$tmpl->addVar('tpl_form', 'comments-form-captcha-html', $captchaHTML);
					} else {
						$tmpl->addVar('tpl_form', 'comments-form-captcha', 0);
					}
				}
			}
