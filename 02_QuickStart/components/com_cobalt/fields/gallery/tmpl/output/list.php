...
$this->record = @$record;
$this->_init();
JFactory::getDocument()->addStyleSheet(JURI::root(TRUE) . '/components/com_cobalt/fields/gallery/gallerybox/thumb-themes/' . $this->params->get('params.thumbs_list_theme', 'default.css'));
...
foreach($this->value as $picture_index => $file)
{
	...
	if($rel)
	{
		...
		if($i <= 2) {
			$out[] = '<a href="'.$url_orig.'" class="image-wrapper-thumb" '.$rel.' id="'.$array_keys[$file['filename']].'"><img src="'.$url.'" class="img-polaroid" alt="" border="0"></a>';
		} else {
			$out2[] = '<a href="'.$url_orig.'" class="image-wrapper-thumb" '.$rel.' id="'.$array_keys[$file['filename']].'"><img src="'.$url.'" class="img-polaroid" alt="" border="0"></a>';
		}
	}
	else
	{
		if($i <= 2) {
			$out[] = '<a href="'.$record->url.'" class="image-wrapper-thumb"><img src="'.$url.'" class="img-polaroid" alt="" border="0"></a>';
		}
	}
	$i++;
}
