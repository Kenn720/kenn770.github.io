...
class CobaltModelRecord extends JModelItem
{
	...
	private function _controls($record, $type, $section, $notitle = FALSE)
	{
		...
		if($notitle)
		{
			$pattern        = '<a href="%s"><img border="0" src="' . JURI::root(TRUE) . '/media/mint/icons/16/%s" alt="%s" align="absmiddle" title="%s" /></a>';
			$confirm_patern = '<a href="%s" onclick="javascript:if(!confirm(\'%s\')){return false;}"><img border="0" src="' . JURI::root(TRUE) . '/media/mint/icons/16/%s" alt="%s" align="absmiddle" title="%s" /></a>';
		}
		else
		{
			$pattern        = '<a href="%s"><img border="0" src="' . JURI::root(TRUE) . '/media/mint/icons/16/%s" alt="%s" align="absmiddle" /> %s</a>';
			$confirm_patern = '<a href="%s" onclick="javascript:if(!confirm(\'%s\')){return false;}"><img border="0" src="' . JURI::root(TRUE) . '/media/mint/icons/16/%s" alt="%s" align="absmiddle" /> %s</a>';
		}
		$out = array();
		if(!$user->get('id'))
		{
		...
		{
			$out[] = sprintf($confirm_patern, Url::task('records.depost', $record->id), addslashes(JText::_('CMSG_DEPOST')), 'arrow-detweet.png', JText::_('CMSG_DEPOST'), JText::_('CMSG_DEPOST'));
		}
		...
		{
			$out[] = sprintf($pattern, Url::task('records.prolong', $record->id), 'clock--plus.png', JText::sprintf('CPROLONG', $type->params->get('properties.default_extend')), JText::sprintf('CPROLONG', $type->params->get('properties.default_extend')));
		}
		...
		if(MECAccess::allowFeatured($record, $type, $section))
		{
			...
			$out[] = sprintf($pattern, Url::task('records.' . ($record->featured ? 'sunfeatured' : 'sfeatured'), $record->id),
		...
		if(MECAccess::allowCommentBlock($record, $type, $section))
		{
			...
			$out[]   = sprintf($pattern, Url::task('records.' . ($enabled ? 'commentsdisable' : 'commentsenable'), $record->id), ($enabled ? 'balloon--minus.png' : 'balloon--plus.png'), ($enabled ? JText::_('CDISABCOMM') : JText::_('CENABCOMM')), ($enabled ? JText::_('CDISABCOMM') : JText::_('CENABCOMM')));
		}

		if(MECAccess::allowPublish($record, $type, $section))
		{
			$out[] = sprintf($pattern, Url::task('records.' . ($record->published ? 'sunpub' : 'spub'), $record->id), ($record->published ? 'cross-circle.png' : 'tick.png'), ($record->published ? JText::_('CUNPUB') : JText::_('CPUB')), ($record->published ? JText::_('CUNPUB') : JText::_('CPUB')));
		}

		if(MECAccess::allowHide($record, $type, $section))
		{
			$out[] = sprintf($pattern, Url::task('records.' . ($record->hidden ? 'sunhide' : 'shide'), $record->id), ($record->hidden ? 'eye-half0.png' : 'eye-half.png'), ($record->hidden ? JText::_('CUNHIDE') : JText::_('CHIDE')), ($record->hidden ? JText::_('CUNHIDE') : JText::_('CHIDE')));
		}

		if(MECAccess::allowDelete($record, $type, $section) && $view != 'record')
		{
			$out[] = sprintf($confirm_patern, Url::task('records.delete', $record->id), addslashes(JText::_('CCONFIRMDELET_1')), 'minus-circle.png', JText::_('CDELETE'), JText::_('CDELETE'));
		}
		if(MECAccess::allowDelete($record, $type, $section) && $view == 'record')
		{
			...
			$out[] = sprintf($confirm_patern, Url::task('records.delete', $record->id, $return), addslashes(JText::_('CCONFIRMDELET_1')),
		...
		if(MECAccess::allowAuditLog($section))
		{
			...
			if($lognums[$record->id])
			{
				...
				$out[] = sprintf($pattern, JRoute::_($url), 'calendar-list.png', JText::_('CAUDITLOG'), JText::_('CAUDITLOG') . " ({$lognums[$record->id]})");
			}
		}
		if(MECAccess::allowRollback($record, $type, $section) || MECAccess::allowCompare($record, $type, $section))
		{
			...
			{
				$label   = sprintf($pattern, 'javascript:void(0);', 'arrow-split-090.png', JText::_('CVERCONTRL'), JText::_('CVERCONTRL') . ' - v.' . $record->version);
				...
				foreach($vernums[$record->id] AS $version)
				{
					...
					{
						$out[$label][$ver][] = sprintf($pattern, Url::task('records.rollback', $record->id . '&version=' . $version->version), 'arrow-merge-180-left.png', JText::_('CROLLBACK'), JText::_('CROLLBACK'));
					}
					...
					if(MECAccess::allowCompare($record, $type, $section))
					{
						...
						$out[$label][$ver][] = sprintf($pattern, $url, 'blue-document-view-book.png', JText::_('CCOMPARECUR'), JText::_('CCOMPARECUR'));
					}
				}

				$url           = 'index.php?option=com_cobalt&view=versions&record_id=' . $record->id . '&return=' . Url::back();
				$out[$label][] = sprintf($pattern, $url, 'drawer.png', JText::_('CVERSIONSMANAGE'), JText::_('CVERSIONSMANAGE'));
			}
		}
		...
		if(MECAccess::allowModerate($record, $type, $section))
		{
			...
			$out[] = sprintf($pattern, JRoute::_($url), 'user-share.png', JText::_('CSETMODER'), JText::_('CSETMODER'));
		}
