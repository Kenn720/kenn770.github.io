...
<div class="page-header">
	<h1><?php echo JText::_('CAUDITLOG')?></h1>
</div>

<form action="<?php echo JRoute::_('index.php?option=com_cobalt&view=auditlog&Itemid='.JFactory::getApplication()->input->getInt('Itemid')); ?>" method="post" name="adminForm" id="adminForm">
	...
	<div class="controls controls-row">
		<div class="input-append pull-left">
			...
			<button class="btn<?php echo ($this->state->get('filter.search') ? ' btn-warning' : NULL); ?>" type="button" onclick="Cobalt.setAndSubmit('filter_search', '');" rel="tooltip" data-original-title="<?php echo JText::_('JSEARCH_FILTER_CLEAR'); ?>">
				<?php echo HTMLFormatHelper::icon('eraser.png');  ?>
			</button>
			<?php endif; ?>
			<button class="btn<?php echo (($this->state->get('auditlog.section_id') || $this->state->get('auditlog.type_id')
		...
		<!--
		<button type="button" class="btn pull-right" onclick="Cobalt.removeLog();">
		...
		<tbody>
			...
			<tr class=" <?php echo $k = 1 - @$k?>" id="row-<?php echo $item->id ?>">
				...
				<td class="has-context">
					...
					<?php endif;?>
					<?php IF($item->event == ATlog::REC_EDIT):?>
						<a class="btn btn-mini" rel="tooltip" data-original-title="<?php echo JText::_('CCOMPAREVERSION');?>"
							href="<?php echo $url = 'index.php?option=com_cobalt&view=diff&record_id=' . $item->record_id . '&version=' . ($params->version) . '&return=' . Url::back(); ?>">
					...
					<a onclick="Cobalt.setAndSubmit('filter_search', 'rid:<?php echo $item->record_id;?>');" href="javascript:void(0);" rel="tooltip" data-original-title="<?php echo JText::_('CFILTERTIPRECORD')?>;">
						<?php echo HTMLFormatHelper::icon('funnel-small.png');  ?></a>
					<div>
						<small>
							<?php echo JText::_('CTYPE'); ?>:
							<a href="#" rel="tooltip" data-original-title="<?php echo JText::_('CFILTERTIPETYPE');?>"
									onclick="Cobalt.checkAndSubmit('#ftype<?php echo $item->type_id; ?>', <?php echo $item->type_id; ?>)">
							...
							<a href="#" rel="tooltip" data-original-title="<?php echo JText::_('CFILTERTIPSECTION');?>"
									onclick="Cobalt.checkAndSubmit('#fsection<?php echo $item->section_id; ?>', <?php echo $item->section_id; ?>)">
				...
				<td nowrap>
					...
					<a onclick="Cobalt.checkAndSubmit('#fuser<?php echo $item->user_id; ?>', <?php echo $item->section_id; ?>)" href="javascript:void(0);" rel="tooltip" data-original-title="<?php echo JText::_('CFILTERTIPUSER')?>">
						<?php echo HTMLFormatHelper::icon('funnel-small.png');  ?></a>
				</td>
			</tr>
				<?php endforeach;?>
