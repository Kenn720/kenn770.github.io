...
class CEventsHelper
{
	...
	static public function notification($event, $filter)
	{
		...
		$filter_pattern = ' <a onclick="Cobalt.setAndSubmit(\'filter_search\', \'%s:%s\')" href="javascript:void(0);"
		...
		if($filter)
		{
			...
			$section_link .= ' <a onclick="Cobalt.setAndSubmit(\'section_id\', \'' . $section->id . '\');" href="javascript:void(0);"
	...
	static public function get_notification($item, $filter = TRUE)
	{
		...
		{
			$db = JFactory::getDbo();
			include_once JPATH_ROOT . '/administrator/components/com_cobalt/tables/field.php';
			$field = new CobaltTableField($db);
			$field->load($item->ref_5);
			$field->params = new JRegistry($field->params);

			include_once JPATH_ROOT . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_cobalt' . DIRECTORY_SEPARATOR . 'fields' . DIRECTORY_SEPARATOR . $field->field_type . DIRECTORY_SEPARATOR . $field->field_type . '.php';
