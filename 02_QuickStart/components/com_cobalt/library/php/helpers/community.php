...
class CCommunityHelper
{
	...
	static public function getAvatar($user_id, $width, $height)
	{
		...
		if(!$files[$user_id])
		{
			...
			if($email && JComponentHelper::getParams('com_cobalt')->get('gravatar'))
			{
			...
			else
			{
				if(!$default)
				{
					if($user_id)
					{
						$default = JPATH_ROOT . '/components/com_cobalt/images/avatar1.gif';
					}
					else
					{
						$default = JPATH_ROOT . '/components/com_cobalt/images/avatar0.gif';
					}
				}
	...
	static public function getName($id, $section, $options = array())
	{
		...
		if(!$options->get('nobadge'))
		{
				...
				!in_array($section->params->get('personalize.novip'), JFactory::getUser($id)->getAuthorisedViewLevels()) && JFile::exists($api))
			{
				...
				if(!empty($icon))
				{
					$name .= ' <img src="' . JURI::root(TRUE) . '/components/com_cobalt/images/vip/' . $icon . '" alt="VIP" />';
				}
			}
			...
			{
				$name .= ' <img src="' . JURI::root(TRUE) . '/components/com_cobalt/images/moderator/' . $icons[$section->id][$id]->icon . '" alt="" />';
			}
		}
		...
		if($section->params->get('personalize.onlinestatus', 1) && !$options->get('noonlinestatus'))
		{
			...
			$name = '<img alt="" src="' . JURI::root(TRUE) . '/media/mint/icons/16/status' . ($result ? NULL : '-offline') . '.png" rel="tooltip" data-original-title="' . ($result ? JText::_('CONLINE') : JText::_('COFFLINE')) . '">' . $name;
		}
	...
	static private function _getClass($component = NULL)
	{
		...
		if(!$component)
		{
			...
			$component = str_replace('.php', '', $params->get('community', 'com_cobalt.php'));
		}
		...
		if(empty($class[$component]))
		{
			...
			{
				$component = 'com_cobalt';
			}
			$file = JPATH_ROOT . '/components/com_cobalt/library/php/community/' . $component . '/' . $component . '.php';
