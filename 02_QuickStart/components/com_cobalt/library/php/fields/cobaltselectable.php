...
class CFormFieldSelectable extends CFormField
{
	...
	public function onFilterWornLabel($section)
	{
		...
		foreach($value as $val)
		{
			$c = explode($this->params->get('params.color_separator', '^'), urldecode($val));
	...
	protected function _custom_value($type)
	{
		...
		$out[] = sprintf('<div class="variant-container" id="variant_%d">
			<a class="small" id="show_variant_link_%d" rel="{field_type:\'%s\', id:%d, inputtype:\'%s\', width:%d, limit:%d, max_size:%d}"
				href="javascript:void(0)" onclick="Cobalt.showAddForm(this)">%s</a></div>', $this->id, $this->id, $this->type, $this->id, $type, $this->params->get('params.width', '220'), $this->params->get('params.total_limit', 0), $this->params->get('params.size', 0), JText::_($this->params->get('params.user_value_label', 'Your variant')));
		$out[] = '<div style="clear:both"></div>';
	...
	public function onFilterGetValues($post)
	{
		...
		foreach($list as $k => $item)
		{
			...
			$out[] = array($item->text, $label, $label, $label_num); //, $text, $text);
		}
	...
	{
		return $this->_import_fieldlist($heads, $defaults);
	}
}
