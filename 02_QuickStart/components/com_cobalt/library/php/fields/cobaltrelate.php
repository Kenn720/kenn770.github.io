...
class CFormFieldRelate extends CFormField
{
	...
	protected function _render_input($type, $name, $section_id, $types, $multi = TRUE)
	{
		...
		if(count($list) == 1 && $this->type == 'child' && ($this->params->get('params.multi_parent') == 0))
		{
			$default = $list;
			$type    = 10;
		}
		...
		switch($type)
		{
			case 2:
				if($this->value)
				{
					$query = $db->getQuery(TRUE);
					$query->from('#__js_res_record');
					$query->select('id, title as html, null as plain, title as render');
	...
	protected function _render_autocomplete($multi, $list, $default, $limit, $name)
	{
		...
		$options['ajax_data']      = "field_id:{$this->id},func:'onGetList',field:'{$this->type}',record_id:" . $app->input->getInt('id', 0) . ",section_id:" . $app->input->getInt('section_id');
		...
		$html[] = JHtml::_('mrelements.listautocomplete', $name, "field_" . $this->id, $default, $list, $options);
	...
	protected function _render_checkbox($multi, $list, $default, $name)
	{
		...
		if($multi)
		{
			$type = 'checkbox';
			$name .= '[]';
			$attribs .= ' onchange="Cobalt.countFieldValues(jQuery(this), ' . $this->id . ', ' . $this->params->get('params.multi_limit', 0) . ', \'checkbox\');"';
		}
	...
	protected function _render_popup($multi, $default, $section_id, $type_id, $name)
	{
		...
		$doTask = JURI::root(TRUE) . '/index.php?option=com_cobalt&view=elements&layout=records&tmpl=component&section_id=' .
	...
	protected function _render_select($multi, $list, $default, $name)
	{
		...
		if($multi)
		{
			...
			$attribs .= ' onchange="Cobalt.countFieldValues(this, ' . $this->id . ', ' . $this->params->get('params.multi_limit', 0) . ', \'select\');"';
	...
	protected function _render($client, $record, $field_id, $type_id, $section_id, $view_what)
	{
		...
		if($this->content['html'] && ($this->content['total'] >= $this->params->get('params.limit_' . $client, 10)) && in_array($this->params->get('params.show_list_all'), JFactory::getUser()->getAuthorisedViewLevels()) && $this->params->get('params.show_list_all_' . $client))
		{
			...
			$file = JPATH_ADMINISTRATOR . '/components/com_cobalt/tables/field.php';
			require_once $file;
			$db    = JFactory::getDbo();
			$field = new CobaltTableField($db);
	...
	public function validate($value, $record, $type, $section)
	{
		...
		{
			if($this->params->get('params.input_mode') == 1 || $this->params->get('params.input_mode') == 2)
			{
				$value = explode(',', $value);
			}
			if(count($value) > $this->params->get('params.multi_limit'))
			{
	...
	public function onAttachExisting($params, $record)
	{
		...
		$save['type_id']     = ($params['field'] == 'child' ?
			$this->type_id : JModelLegacy::getInstance('Fields', 'CobaltModel')->getFieldTypeId($save['field_id']));
	...
	public function onImportForm($heads, $defaults)
	{
		...
		$type    = ($this->type == 'child' ? $this->params->get('params.parent_type') : JModelLegacy::getInstance('Fields', 'CobaltModel')->getFieldTypeId($this->params->get('params.child_field')));
