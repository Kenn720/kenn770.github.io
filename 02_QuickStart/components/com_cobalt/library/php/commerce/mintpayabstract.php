...
class MintPayAbstract
{
	...
	{
		$this->table = JTable::getInstance('Sales', 'CobaltTable');
	}
	...
	public function receive($field, $post, $record)
	{
		if($field->params->get('params.send_mail'))
		{
			...
			$body = str_replace('[ORDERS]', JHtml::link(JUri::root().'index.php?option=com_cobalt&view=elements&layout=buyer', JText::_('CORDERHIST')), $body);
	...
	public function log($msg, $add = NULL)
	{
		...
		$folder = JPATH_ROOT. DIRECTORY_SEPARATOR .'logs'. DIRECTORY_SEPARATOR .$this->provider;
