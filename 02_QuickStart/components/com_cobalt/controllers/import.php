...
class CobaltControllerImport extends JControllerAdmin
{
	...
	public function import()
	{
		...
		try
		{
			...
			$params     = new JRegistry($imports->params);
			$conversion = new JRegistry($imports->crossids);
			...
			foreach($list AS $record)
			{
				...
				$data['ctime']   = JDate::getInstance($row->get($params->get('field.ctime', 'now')))->toSql();
				$data['user_id'] = $row->get($params->get('field.user_id'), JFactory::getUser()->get('id'));
				$data['access']  = $row->get($params->get('field.access'));
				$data['extime']  = $row->get($params->get('field.extime'));
