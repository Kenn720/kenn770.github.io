...
class CobaltControllerForm extends JControllerForm
{
	...
	public function save($key = NULL, $urlVar = NULL)
	{
		...
		if($record['id'])
		{
			CEmeraldHelper::allowType('edit', $type, $user_id, $section, TRUE,
				'index.php?option=com_cobalt&view=form&id=' . $record['id'], @$record['user_id'], FALSE);
		...
		else
		{
			CEmeraldHelper::allowType('submit', $type, $user_id, $section, TRUE,
				'index.php?option=com_cobalt&view=form&section_id=' . $section->id . '&type_id=' . $type->id, @$record['user_id'], FALSE);
			$this->text_prefix = $type->params->get('submission.submit_msg', 'JLIB_APPLICATION');
		}

		$app = JFactory::getApplication();
		if(!$app->getUserState('com_cobalt.edit.form.id'))
		{
			$app->setUserState('com_cobalt.edit.form.id', $record['id']);
		}
	...
	public function postSaveHook(JModelLegacy $model, $validData = array())
	{
		...
		*/
		...
		ArrayHelper::clean_r($out);

		if((boolean)$out)
		{
			$sql = 'DELETE FROM #__js_res_tags_history WHERE record_id = ' . $record_id . ' AND tag_id NOT IN (' . implode(',', $out) . ')';
			$db->setQuery($sql);
			$db->query();
		}
		...
		if($section->params->get('personalize.personalize') && $section->params->get('personalize.post_anywhere'))
		{
			...
			if($posts)
			{
				...
				foreach($posts as $pid)
				{
					...
					$post_table = JTable::getInstance('Reposts', 'CobaltTable');
		...
		if($record->parent_id && $type->params->get('properties.rate_access') == -1)
		{
			$query = "SELECT COUNT(*) as total, SUM(votes_result) / COUNT(*) AS rating
                FROM #__js_res_record WHERE parent_id = $record->parent_id AND parent = 'com_cobalt'";
			$db->setQuery($query);
			$new = $db->loadObject();
		...
		if(!$isnew)
		{
			...
			// If cobalt article is acting lice a comment we have to add event alert that new comment has been added
			if($record->parent_id && $record->parent == 'com_cobalt')
			{
		...
		else
		{
			...
			if($this->input->getInt('fand') && $this->input->getInt('field_id'))
			{
				...
				$fand = JTable::getInstance('Record', 'CobaltTable');
			...
			if($record->parent_id && $record->parent == 'com_cobalt')
			{
				...
				/*$db->setQuery("SELECT COUNT(id) FROM `#__js_res_record` WHERE parent_id = {$record->parent_id} AND parent = 'com_cobalt' AND published = 1");
	...
	{
		$user  = JFactory::getUser();
		$allow = $user->authorise('core.create', 'com_cobalt.record');
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		//return JFactory::getUser()->authorise('core.edit', 'com_cobalt.record');
	}
