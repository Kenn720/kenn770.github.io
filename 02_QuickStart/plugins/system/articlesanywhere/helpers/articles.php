...
class PlgSystemArticlesAnywhereHelperArticles
{
	...
	public function replace(&$string, &$match, $art = null, $type = '')
	{
		...
		$this->helpers->get('tags' . $type)->data->article = $art;
		$this->helpers->get('tags' . $type)->data->total   = $total;
		...
		foreach ($ids as $i => $id)
		{
			...
			$this->helpers->get('tags' . $type)->data->count  = $count;
			$this->helpers->get('tags' . $type)->data->even   = (bool) (!($count % 2));
			$this->helpers->get('tags' . $type)->data->uneven = (bool) ($count % 2);
			$this->helpers->get('tags' . $type)->data->first  = (bool) ($count == 1);
			$this->helpers->get('tags' . $type)->data->last   = (bool) ($count >= $total);
