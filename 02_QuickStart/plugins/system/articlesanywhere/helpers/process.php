...
class PlgSystemArticlesAnywhereHelperProcess
{
	...
	public function getArticlesFromTags(&$matches)
	{
		...
		foreach ($types as $type => $ids)
		{
			...
			$query->select('CONCAT("' . $type . '_", a.id) AS type_id')
				->select('CONCAT("' . $type . '_", a.id) AS orig_id')
				->select('a.*')
				->select('a.access IN(' . implode(', ', $this->aid) . ') as has_access')
				->from('#__' . $table . ' as a');
	...
	private function getArticle($id, $current_article, $type = 'article', $ignores = array())
	{
		$id = $this->getArticleId($id, $current_article, $type, $ignores);
	...
	private function getArticleId($id, $article, $type = 'article', $ignores = array())
	{
		...
		{
			return $this->getCurrentArticleId($article, $type);
		}

		return $this->getArticleIdById($id, $type, $ignores);
	}

	private function getCurrentArticleId($article, $type = 'article')
	{
		if (isset($article->id))
		{
			return $article->id;
		...
		if (isset($article->link) && preg_match('#&(?:amp;)?id=([0-9]*)#', $article->link, $match))
		{
			return $match['1'];
		}

		if ($type != 'k2' && $this->params->option == 'com_content' && JFactory::getApplication()->input->get('view') == 'article')
		{
			return JFactory::getApplication()->input->getInt('id', 0);
		}

		if ($type == 'k2' && $this->params->option == 'com_k2' && JFactory::getApplication()->input->get('view') == 'item')
		{
			return JFactory::getApplication()->input->getInt('id', 0);
		}
	...
	private function getArticleIdById($id, $type = 'article', $ignores = array())
	{
		if (isset($this->articles[$type . '_' . $id]))
		{
			return $id;
