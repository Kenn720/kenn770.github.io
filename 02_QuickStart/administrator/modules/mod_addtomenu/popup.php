...
class AddToMenu
{
			...
			foreach ($template->extras->extra as $element)
			{
				if ($element->type == 'toggler')
				{
					if (isset($element->param))
					{
						...
						$el = '</div></div><div id="' . rand(1000000, 9999999) . '___' . implode('___', $ids) . '" class="rl_toggler rl_toggler_horizontal"><div><div>';
					}
					else
					{
						$el = '</div></div></div>';
	...
	function outputHTML(&$template, &$elements)
	{
		...
		RLFunctions::script('regularlabs/toggler.min.js', '16.4.13421');
