...
RLFunctions::stylesheet('regularlabs/style.min.css', '16.4.18881');
?>
<form action="<?php echo JRoute::_('index.php?option=com_advancedtemplates'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<div id="j-main-container" class="span10">
		...
		{
			echo RLVersions::render('ADVANCED_TEMPLATE_MANAGER');
		}
		?>
					...
					<?php foreach ($this->items as $i => $item) :
						...
						<tr class="row<?php echo $i % 2; ?>">
							...
							<td>
								...
								{
									$tooltip .= '<br><em>' . htmlspecialchars(JText::sprintf('JGLOBAL_LIST_NOTE', $this->escape($item->params->note))) . '</em>';
								}
								...
								<?php if (!empty($item->params->note) && $this->config->show_note == 2) : ?>
									<div class="small">
										<?php echo JText::sprintf('JGLOBAL_LIST_NOTE', $this->escape($item->params->note)); ?>
									</div>
								<?php endif; ?>
							</td>
							<?php if ($this->config->show_note == 3) : ?>
		...
		echo RLLicenses::render('ADVANCED_TEMPLATE_MANAGER');

		// Copyright
		echo RLVersions::getFooter('ADVANCED_TEMPLATE_MANAGER');
		?>
	</div>
</form>
