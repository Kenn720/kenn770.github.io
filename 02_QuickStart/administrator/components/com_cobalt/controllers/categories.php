...
class CobaltControllerCategories extends JControllerAdmin
{
	...
	function getModel($name = 'Category', $prefix = 'CobaltBModel', $config = array('ignore_request' => true))
	{
	...
	public function rebuild()
	{
		...
		$this->setRedirect(JRoute::_('index.php?option=com_cobalt&view=categories&section_id='.$section, false));
		...
		if ($model->rebuild()) {
			// Rebuild succeeded.
			$this->setMessage(JText::_('COM_COBALT_REBUILD_SUCCESS'));
			return true;
		} else {
			// Rebuild failed.
			$this->setMessage(JText::_('COM_COBALT_REBUILD_FAILURE'));
			return false;
		}
	}
